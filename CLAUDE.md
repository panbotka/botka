...
- **Force-run escape hatch:** `tick()` refuses to pick *any* task while either rate-limit gate is active — the `UsageMonitor` 5h/7d threshold (`USAGE_THRESHOLD_5H`, default 0.90) or the `RateLimitGate` cooldown. Since the usage gate trips at 90%, not 100%, a queued task can sit blocked while Claude would still answer. `Runner.ForceRunTask` (`POST /api/v1/tasks/:id/force-run`, the amber **"Spustit teď"** button shown only on `queued` tasks in the task detail page) is the manual override: it claims the task via `pickTaskByID` — a gate-free, by-ID sibling of `pickNextTask` — and launches it, **never consulting either gate**. It is deliberately *per-task*: the gates stay armed, so every other queued task remains blocked. It still enforces the hard invariants and refuses with a typed sentinel otherwise (`ErrRunnerStopped` when hard-stopped, `ErrWorkersBusy`, `ErrProjectBusy`, `ErrTaskNotQueued`), all mapped to `409`. One-task-per-project survives the TOCTOU window between the in-memory pre-flight and the claim because `pickTaskByID` carries the same `NOT EXISTS` subquery and `FOR UPDATE SKIP LOCKED`, with the `idx_one_running_per_project` unique index as the backstop. A forced task runs through the normal `executeTask` path, so if it fails on a genuine rate-limit error the gate simply re-arms itself.
